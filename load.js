document.addEventListener("DOMContentLoaded", function () {
  ...
  const infoText = createElement("p", {
    innerHTML: "Hello! This is still in a testing phase, and it is impossible to create an account, nor upload. However, you can enjoy the work we are putting into it by exploring the site. Goodbye!",
  ...
  function showOptions() {
    ...
    showLoadingScreen(optionsMenu);

    setTimeout(() => {
      const optionsContent = createElement("div", {
        style: `background: ${isDarkMode ? "#1a1a1a" : "white"}; padding: 20px; border-radius: 10px; color: ${isDarkMode ? "#ffffff" : "#000000"}; transition: background 0.3s ease-in-out, color 0.3s ease-in-out;`,
      });
  ...
  function showLoadingScreen(parentElement) {
    const loadingScreen = createElement("div", {
      class: "loading-screen",
    });

    const loadingText = createElement("p", {
      innerHTML: "Changing themes...",
    });

    const loadingWheel = createElement("div", {
      class: "loading-wheel",
    });

    loadingScreen.append(loadingText, loadingWheel);
    (parentElement || document.body).appendChild(loadingScreen);
  }

  function hideLoadingScreen(parentElement) {
    const loadingScreen = document.querySelector(".loading-screen");
    if (loadingScreen) {
      loadingScreen.style.opacity = "0";
      setTimeout(() => {
        (parentElement || document.body).removeChild(loadingScreen);
      }, 300);
    }
  }
});
